...
test.describe('Task management', () => {
  test('create a standalone task and view it on My Tasks page', async ({ page }) => {
    ...
    // Navigate to employees and get a subject to create a task for
    await page.goto('/employees');
    const firstEmployee = page.getByRole('link').filter({ hasText: /\w+ \w+/ }).first();
    await firstEmployee.click();
    await page.waitForURL('**/employees/**');

    // Navigate to My Tasks via home
    ...
    const subjectSelect = page.getByLabel('Relates to (employee)');
